...
## Math
-  수학적인 상수와 함수를 위한 속성과 메서드를 가진 내장 객체. 함수 객체가 아니다
- Math는 `Number` 자료형만 지원하며 `BigInt`와는 사용할 수 없다.
- 다른 전역 객체와 달리 Math는 생성자가 아님
- Math의 모든 속성과 메서드는 정적
- 파이 상수는 Math.PI로 참조할 수 있고, 사인 함수는 매개변수 x에 대해 Math.sin(x)와 같이 호출할 수 있음
...
## Math의 메서드
- 참고: 삼각 함수(`sin()`, `cos()`, `tan()`, `asin()`, `acos()`, `atan()`, `atan2()`)는 매개변수와 반환값 모두 호도법(라디안)을 사용
- 참고: 많은 수의 `Math 함수` 정확도는 구현에 따라 다를 수 있음. 즉, 각 브라우저의 결과가 다를 수 있으며, 서로 같은 JS 엔진이라도 운영체제나 아키텍쳐에 따라서 불일치하는 값을 반환할 수 있음

|메서드      |      반환|
|------------|------------------------------------------------------------------|
|Math.abs(x)|숫자의 절댓값을 반환|
|Math.acos(x)|숫자의 아크코사인 값을 반환|
|Math.acosh(x)|숫자의 쌍곡아크코사인 값을 반환|
|Math.asin(x)|숫자의 아크사인 값을 반환|
|Math.asinh(x)|숫자의 쌍곡아크사인 값을 반환|
|Math.atan(x)|숫자의 아크탄젠트 값을 반환|
|Math.atanh(x)|숫자의 쌍곡아크탄젠트 값을 반환|
|Math.atan2(y, x)|인수 몫의 아크탄젠트 값을 반환|
|Math.cbrt(x)|숫자의 세제곱근을 반환|
|Math.ceil(x)|인수보다 크거나 같은 수 중에서 가장 작은 정수를 반환|
|Math.clz32(x)|주어진 32비트 정수의 선행 0 개수를 반환|
|Math.cos(x)|숫자의 코사인 값을 반환|
|Math.cosh(x)|숫자의 쌍곡코사인 값을 반환|
|Math.exp(x)|E의 x승을 반환. x는 인수이며 E 는 오일러 상수(2.718...) 또는 자연로그의 밑|
|Math.expm1(x)|exp(x)에서 1을 뺀 값을 반환|
...
|Math.log(x)|숫자의 자연로그(loge 또는 ln) 값을 반환|
|Math.log1p(x)|숫자 x에 대해 1 + x의 자연로그(loge 또는 ln) 값을 반환|
|Math.log10(x)|숫자의 밑이 10인 로그를 반환|
|Math.log2(x)|숫자의 밑이 2인 로그를 반환|
...
|Math.sign(x) |x의 양의 수인지 음의 수인지 나타내는 부호를 반환|
|Math.sin(x)|숫자의 사인 값을 반환|
|Math.sinh(x)|숫자의 쌍곡사인 값을 반환|
|Math.sqrt(x)|숫자의 제곱근을 반환|
|Math.tan(x)|숫자의 탄젠트 값을 반환|
|Math.tanh(x)|숫자의 쌍곡탄젠트 값을 반환|
...
- 브라우저는 HTML 문서를 읽으면서 script tag를 실행함
  - HTML 문서 Element 파싱보다 스크립트가 먼저 실행되면 접근되지 않음
  - 페이지의 상단의 script에서 파싱될 Element에 접근하고 싶다면 onload 등의 eventhandler로 실행되도록 해야함
- async, defer 속성을 이용하면 스크립트의 실행과 문서의 로딩을 동시에 진행할 수 있음(비 동기적)
- 꼭 필요한 경우가 아니라면 상단보다 하단에 적는 것이 로딩 속도를 더 빠르게 할 수 있음
...
2. 개발자 도구 Sources tab 활용
  ...
  - 단계적 실행
    ...
    - resume 버튼을 통해 프로그램을 계쏙 진행할 수 있음 (다음에 breakpoint를 만나면 다시 중지)
  - Watch 창
...
## 익명함수와 함수 즉시 호출
1. 익명함수의 필요성
  - Javascript에서는 함수가 인자로 사용되거나 콜백 함수로 사용될 수 있기 때문에, 일회성으로 사용될 코드의 블럭을 함수로 정의하는 경우가 많음. 즉, 이러한 함수의 경우 이름이 꼭 필요하지 않음
...
## 예외 처리 - 예외처리의 필요성
1. 프로그램의 외부 요인으로 프로그램의 특정 파트가 정상적으로 동작하지 않을 수 있으나, 에러처리를 이용하면 프로그램이 정상적으로 실행될 수 있음

2. Try, Catch, Finally 구문
````javascript
try {
  // 정상적으로 실행되는 경우 실행될 프로그램 작성
  // try 블록 안에서 에러가 발생한 경우 catch 블록으로 이동함
}
catch(e){
  // try 블록에서 에러가 발생한 경우
...
3. 에러 발생 시키기
  - throw 명령 사용: return 구문과 비슷하게 에러를 나타낼 수 있는 인자를 사용
    - 문자열, 숫자, 객체 등 javascript object를 자유롭게 활용가능
...
*/
//try 구문은 꼭 있어야 하고, catch(e), finally는 둘중에 하나만 있어도 됨
//try 구문이 없으면 시멘틱 에러가 생김
````
````javascript
try {
  console.log("try - 1");
