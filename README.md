...
8. :sun_with_face: 일반적으로 명령문 1을 사용하여 루프에 사용된 변수를 초기화 함(i=0) 항상 그런것은 아니지만 명령문 1은 선택 사항이며 많은 값을 시작할 수 있음. 쉼표로 구분이 가능하다
9. :sun_with_face: 명령문 2는 종종 조기 변수의 조건을 평가하는데 사용됨 명령문 2도 선택사항이며 true를 반환하면 루프가 다시 시작되고 false를 반환하면 루프가 종료됨.
10. 명령문 2을 생략하려면 루프 내부에 break를 제공해야함. 그렇지 않으면 루프가 끝나지 않음. 브라우저가 다운되게 됨
11. :sun_with_face: 명령문 3은 종종 초기 변수 값을 증가 시킴. 명령문 3은 선택사항이며 음수 증분(i--), 양수 증분 (i=i + 15) 또는 기타 다른 작업도 수행이 가능. 루프 내에서 값을 증가시킬 때와 같이 명령문 3도 생략이 가능
